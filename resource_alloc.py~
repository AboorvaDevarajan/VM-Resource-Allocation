...
resource_alloc = Flask(__name__)
resource_alloc.config['UPLOAD_FOLDER'] = 'uploads/'
resource_alloc.config['ALLOWED_EXTENSIONS'] = set(['jpg', 'jpeg','png','JPG'])


@resource_alloc.route('/')
@resource_alloc.route('/index')
def index():
    return render_template('index.html')


@resource_alloc.route('/translate')
def translate():
...
@resource_alloc.route('/upload', methods=['POST'])
def upload():
...
@resource_alloc.route('/uploads/<filename>')
def uploaded_file(filename):
    return send_from_directory(resource_alloc.config['UPLOAD_FOLDER'],
                               filename)
if __name__ == "__main__":
    resource_alloc.run(debug=True)
